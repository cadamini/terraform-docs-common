...
`hosted-json-state-upload-url`   | A URL to which you can upload the state data, in a [stable format](/terraform/internals/json-format) appropriate for external integrations to consume. Only available if the JSON state content has not been uploaded.
`hosted-state-upload-url`        | A URL to which you can upload the state data, in the format used internally by Terraform. Only available if the state content has not been uploaded.
...
`provisional`                    | A boolean flag that denotes a state version that was created but has not yet marked as the current version for a workspace.
...
`status`                         | Indicates a state version's content upload status, can be `pending`, `finalized` or `discarded`.
...
| `data.attributes.state`              | string  | (nothing) | **Optional** Base64 encoded raw state file. If not included in this request, `status` will be `pending` until state is uploaded to `hosted-state-upload-url`.                                                                                                                                                                                                                                                              |
...
-> **Note:** While you can upload state version content in the same request when creating a state version, we strongly recommended that you upload content directly to the provided URL.

`PUT https://archivist.terraform.io/v1/object/<UNIQUE OBJECT ID>`

**The URLs are provided in the `hosted-state-upload-url` and `hosted-json-state-upload-url` attributes when creating a `state-version` resource. Once uploaded, the URL is hidden on the resource and no longer available.**

### Sample Request

**@filename is the name of Terraform state file you wish to upload.**
